...
import com.lzf.myapplication.ControlActivity;
import com.lzf.myapplication.R;
import com.lzf.myapplication.dmp.DeviceItem;
import com.lzf.myapplication.util.Action;
...
public class DMCControl {
    ...
    public void setVolume(long paramLong, int paramInt) {
        ...
        try {
            ...
            if (localService != null) {
                Log.v("lzf", "paramInt：" + paramInt);
                if (paramInt == CUT_VOC) {
                    Log.v("lzf", "CUT_VOC：" + CUT_VOC);
                    if (paramLong >= 0L) {
                        paramLong -= 1L;
                    } else {
                ...
                if (paramInt == ADD_VOC) {
                    Log.v("lzf", "ADD_VOC：" + ADD_VOC);
                    if (paramLong < 102) {
                        paramLong += 8L;
                    } else {
                        Toast.makeText(activity, "已经是最大音量",
                                Toast.LENGTH_SHORT).show();
                    }
                }
                Log.v("lzf", "paramLong：" + paramLong);
                this.upnpService.getControlPoint().execute(
